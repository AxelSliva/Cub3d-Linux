...
void	ft_dist_order2(t_recup *recup)
{
	...
	{
		recup->s.order[i] = i;
		recup->s.dist[i] = ((recup->ray.posx - recup->sxy[i].x) * (recup->ray.posx - recup->sxy[i].x) + (recup->ray.posy - recup->sxy[i].y) * (recup->ray.posy - recup->sxy[i].y));
	}
}
...
void	ft_draw_spr(t_recup *recup, int y, int texx, int stripe)
{
	...
	while (y < recup->s.drawendy)
	{
		...
		if (recup->texture[4].addr[texy * recup->texture[4].line_length / 4 + texx] != -16777216)
		{
